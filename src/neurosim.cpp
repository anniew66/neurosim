...
struct GranuleParam : public ParamGroup {
  ...
  double chemo_diffusion = 100.0;
  double chemo_decay = 0.01;
  double chemo_secretion_rate = 1.0;
  double step_size = 1.0;
  double synapse_radius = 2.0;
};
...
inline int Simulate(int argc, const char** argv) {
  ...
  source->SetPosition({50, 0, 50});
  source->SetName("ChemoSource");
  source->AddBehavior(new Secretion("Chemo",
                     Param<GranuleParam>::Get()->chemo_secretion_rate));
  rm->AddAgent(source);

...
function write_pytorch_script(synapse_csv_path::String,
    ...
    push!(lines, "E = edge_index.shape[1]")
    push!(lines, "n_pairs = len(set(zip(pre_idx.tolist(), post_idx.tolist())))")
    push!(lines, "print(f'Saved network_graph.pt: N={N} nodes, E={E} edges ',")
    push!(lines, "      f'({n_pairs} unique pairs, avg {E/max(n_pairs,1):.1f} multi-edges/pair)')")
    push!(lines, "")
